...
def get_ago_timetemp(w=WEEK,d=DAY,h=HOUR,m=MINUTE,s=SECOND,**kwags):
    """升级返回多少时间之前

    Args:
        w (int, optional): 默认时间. Defaults to WEEK.
        d (int, optional): 默认时间. Defaults to DAY.
        h (int, optional): 默认时间. Defaults to HOUR.
        m (int, optional): 默认时间. Defaults to MINUTE.
        s (int, optional): 默认时间. Defaults to SECOND.

    Returns:
        str: 返回的日期
    """
    if len(kwags)==0:
        ago_date = dateAgo(week=w,day=d,hour=h,minute=m,second=s)
        timetemp,_ = tim_interconversion(ago_date)
        return timetemp
    week = kwags.get('week')
    day = kwags.get('day')
    hour = kwags.get('hour')
    minute = kwags.get('minute')
    second = kwags.get('second')
    ago_date = dateAgo(week=week if week is not None else 0,
...
def tim_interconversion(tim):
    ...
    if not flag:
        ...
        for fmt in sfmformat:
            try:
                date = time.strftime('%Y-%m-%d',time.localtime())
                ti = date+' '+tim
                fmt = '%Y-%m-%d'+' '+fmt
                struct_time = time.strptime(ti,fmt)
                flag = True
                break
            except Exception as e:
                flag = False
...
def testWriteFile(file,flag = 'w',name='test',hz='json'):
    """测试使用
    w  a+ 
    Args:
        file ([type]): [description]
    """
    with open(f'{name}.{hz}',flag) as f:
        f.write(file)
   


if __name__=='__main__':
    s = tim_interconversion('13:07')
    print(s)
#     # fff(s=2)
